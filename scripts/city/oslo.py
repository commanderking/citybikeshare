...
def run_get_exports(playwright, url, csv_path):
    ...
    for button in csv_buttons.all():
        ...
        # https://data-legacy.urbansharing.com/oslobysykkel.no/2018/12.csv.zip
        desired_filename = button.get_attribute("content").split(".no/")[1].replace("/", "-")
...
def map_legacy_station_id_to_name(stations_df):
    stations_df = stations_df.select(["station_id", "name"]).with_columns([pl.col("station_id").cast(pl.Int64)])
    def inner(df):
        headers = df.columns
        ...
        if "duration" not in headers:
            ...
            df = (
                ...
                .join(station_mapping_df, left_on="start_station_legacy_id", right_on="legacy_id")
                .rename({
                    "new_id": "start_station_id"
                })
                .join(station_mapping_df, left_on="end_station_legacy_id", right_on="legacy_id")
                .rename({
                    "new_id": "end_station_id"
                })
