...
class TestCreateParquet:
    REQUIRED_COLUMNS = {"start_time", "end_time", "start_station_name", "end_station_name"}

    def _load_output(self, context: PipelineContext, source_csv: Path) -> pl.DataFrame:
        parquet_path = context.parquet_directory / source_csv.name.replace(".csv", ".parquet")
    ...
    def test_2018_format_includes_membership_and_demographics(self, boston_context, boston_config):
    ...
    def test_config_param_is_used(self, boston_context, tmp_path):
        ...
        bad_config = load_city_config("boston") | {"processing_pipeline": ["nonexistent_step"]}
...
class TestDetermineHasHeader:
    def test_returns_true_when_first_row_matches_expected_columns(self, tmp_path):
        ...
        assert determine_has_header(str(csv), ["start_time", "end_time", "station_name"]) is True
    ...
    def test_returns_false_when_first_row_is_data(self, tmp_path):
        ...
        assert determine_has_header(str(csv), ["start_time", "end_time", "station_name"]) is False
